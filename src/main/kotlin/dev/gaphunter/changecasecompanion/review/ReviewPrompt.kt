package dev.gaphunter.changecasecompanion.review
 ...
 * used a real number of successful conversions -- never on install,
 * never on a timer. Unlike the detector-based plugins in this catalog
 * (where [recordHit] dedupes by a `path:line` key because the platform
 * re-runs the same inspection pass repeatedly), this plugin has no
 * passive "finding" to dedupe against: every call to [recordHit] here
 * already corresponds to one real conversion (the identifier's case
 * actually changed, never the "Already X" no-op branch). So this
...
object ReviewPrompt {
    ...
    /** How many successful conversions before the prompt shows once. */
    private const val HITS_BEFORE_PROMPT = 5

    private const val KEY_HIT_COUNT = "dev.gaphunter.changecasecompanion.review.hitCount"
    private const val KEY_ANSWERED = "dev.gaphunter.changecasecompanion.review.answered"

    private const val NOTIFICATION_GROUP_ID = "Change Case Companion"

    // Marketplace listing (real ID resolved after first publish).
    private const val MARKETPLACE_URL = "https://plugins.jetbrains.com/plugin/33549-change-case-companion/reviews"

    /**
     * Call this once per real, successful conversion -- never for the
     * "Already X" no-op branch (the text didn't actually change). Safe
